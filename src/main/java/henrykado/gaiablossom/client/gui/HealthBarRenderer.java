...
public class HealthBarRenderer extends Gui {
    ...
    private static final ResourceLocation TINKER_HEARTS = new ResourceLocation("gaiablossom", "textures/newhearts.png");
    ...
    public float getDisplayHealth(float health) {
        if (health > 25) {
            return health + 5;
        } else if (health > 20) {
            return 20 + (health - 20) * 2;
        }
        return health;
    }
    ...
    public void renderHealthbar(RenderGameOverlayEvent.Pre event) {
        ...
        health = (int) getDisplayHealth(health);
        int healthLast = MathHelper.ceiling_float_int(mc.thePlayer.prevHealth);
        healthLast = (int) getDisplayHealth(healthLast);
        float healthMax = Math.min(20F, (float) attrMaxHealth.getAttributeValue());
        healthMax = (int) getDisplayHealth(healthMax);
        ...
        if (health > 20) {
            ...
            // yBasePos -= 20;
            final int heartIndexMax = Math.min(10, (health - 20) / 2);
            ...
            for (int j = 0; j < heartIndexMax; j++) {
                ...
                if (20 + j * 2 + 21 >= health) {
                    // full heart texture
                    this.drawTexturedModalRect(xBasePos + 8 * j, yBasePos + y, 0, tinkerTextureY, 9, 9);
                }
            }
            ...
            if (health % 2 == 1 && heartIndexMax < 10) {
                ...
                // half heart texture
                this.drawTexturedModalRect(xBasePos + 8 * heartIndexMax, yBasePos + y, 9, tinkerTextureY, 9, 9);
            }
            mc.getTextureManager()
                .bindTexture(icons);
        }
